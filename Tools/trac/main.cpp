...
int main(int argc, char *argv[])
{
    ...
    if (cmdargs.Opt.count("-u") > 0)
    {
        ...
        const String dst = cmdargs.PosArgs.size() > 1 ? cmdargs.PosArgs[1] : Path::ReplaceExtension(src, "trs");
        return Command_Decompile(src, dst);
    }
    else
    {
        const String &src = cmdargs.PosArgs[0];
        const String dst = cmdargs.PosArgs.size() > 1 ? cmdargs.PosArgs[1] : Path::ReplaceExtension(src, "tra");
        String game_name;
        int game_uid = 0;
